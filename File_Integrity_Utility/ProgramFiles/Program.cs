    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            while (userChosenMenuOption != 0)
            {
                ...
                ExecuteUserChosenMenuOption(userChosenMenuOption);
                Console.WriteLine();
        ...
        private static int ObtainMenuOptionFromUser()
        {
            ...
            while (userInputAsInt < 0 || userInputAsInt > 8)
            {
        ...
        private static void DisplayMainMenu()
        {
            ...
            Console.WriteLine("4: Given a folder, rename each of its top level file(s) as its respective SHA 256 hash.");
            Console.WriteLine("5: Given a folder, which contains file(s) named after their respective SHA 256 hash, verify each file's hash matches its name.");
            Console.WriteLine("6: Given a folder, generate a text file of SHA 256 hashes for all of its top level files.");
            Console.WriteLine("This text file will be placed within the given folder.");
            Console.WriteLine("7: Given a folder, which already contains a text file of SHA 256 hashes for all of its top level files (that was generated by this program), determine:");
            ...
            Console.WriteLine("8: Given two folders, determine if both are identical:");
        ...
        private static void ExecuteUserChosenMenuOption(int userChosenMenuOption)
        {
            ...
            else if (userChosenMenuOption == 7)
            {
                MenuOption7.CompareHashesOfFilesInFolderToRecordedHashesInTextFile();
            }
            else // (userChosenMenuOption == 8)
            {
                MenuOption8.DisplayIfBothFoldersContainTheSameFilesAndStructure();
            }
        }


        private static void ExecuteUserChosenMenuOptionWithReflection(int userChosenMenuOption)
        {
            // We are using reflection to dynamically call the correct class and method below:
            string pathOfMenuOptionClasses = "File_Integrity_Utility.ProgramFiles.MenuOptions";
            Type? menuOption = Type.GetType(pathOfMenuOptionClasses + ".MenuOption" + userChosenMenuOption);
            // We already know that we will only call MenuOption1 - MenuOption8, which won't return a null reference:
#pragma warning disable CS8602 // Dereference of a possibly null reference.
            MethodInfo? method = menuOption.GetMethod("Execute");
